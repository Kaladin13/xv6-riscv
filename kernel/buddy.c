...
void invert_buddy_bit(char *array, int index) {
...
void bd_print() {
  for (int k = 0; k < nsizes; k++) {
    ...
    printf("  alloc:");
    bd_print_vector(bd_sizes[k].buddy_xor, NBLK(k));
    if (k > 0) {
      printf("  split:");
...
void *bd_malloc(uint64 nbytes) {
  ...
  invert_buddy_bit(bd_sizes[k].buddy_xor, blk_index(k, p));
  ...
  for (; k > fk; k--) {
    ...
    invert_buddy_bit(bd_sizes[k - 1].buddy_xor, blk_index(k - 1, p));
...
void bd_free(void *p) {
  ...
  for (k = size(p); k < MAXSIZE; k++) {
    ...
    invert_buddy_bit(bd_sizes[k].buddy_xor, bi);           // free p at size k
    if (buddy_bit_isset(bd_sizes[k].buddy_xor, buddy)) {  // is buddy allocated?
...
void bd_mark(void *start, void *stop) {
  ...
  for (int k = 0; k < nsizes; k++) {
    ...
    for (; bi < bj; bi++) {
      ...
      invert_buddy_bit(bd_sizes[k].buddy_xor, bi);
    }
  }
}
